...
unless Rails.env.production?
  ...
  organizations.each do |organization_name|
    ...
    meters = ['api calls', 'downloads'].map do |meter_name|
      Meter.find_or_create_by(name: meter_name, organization: org)
    end

    product = Product.find_or_create_by(name: 'Plutto Billings', organization: org, meter: meters.sample)
    ...
    customers.each do |customer_info|
      ...
      Customer.includes(:billing_information).find_or_create_by(
        ...
        (1..12).to_a.reverse.each do |i|
          CreateNewMeterEvent.for(
            timestamp: DateTime.current - (i * 2).days,
            amount: rand(1..1000),
            action: 'increment',
            meter_id: meters.sample.id,
