class AnalyticsService < PowerTypes::Service.new(:organization)
  ...
  def organization_invoices_by_month(currency)
    invoices = invoices(currency)
    {
      ...
      canceled: invoices_amount_per_month(invoices.canceled),
      sent: invoices_amount_per_month(invoices.sent),
  ...
  def subscriptions(currency = nil)
    @subscriptions ||= if currency
                         @organization.subscriptions.where(currency: currency).active.not_zombie
                       else
                         @organization.subscriptions.active.not_zombie
                       end
  end
